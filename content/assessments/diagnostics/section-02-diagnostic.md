...
### D09
<!-- meta: objective=2.5; answer=C -->
**Question:** A source is available only through an ODBC driver and has no suitable managed connector. Which notebook design is safest for an incremental extract?

A. Interpolate a watermark and password into one SQL string, then call `fetchall()`.
B. Run an unbounded full-table query on the driver during every job.
C. Bind a half-open watermark interval as query parameters, fetch in bounded batches, persist progress, and obtain the connection string from secrets.
D. Assume the ODBC client automatically distributes the query across Spark executors.

**Rationale:** ODBC is client-driven; parameter binding, bounded fetches, secret handling, persisted incremental state, and Jobs orchestration are the team's responsibility.

**Reference:** [Connect to data sources](https://docs.databricks.com/aws/en/connect/)
